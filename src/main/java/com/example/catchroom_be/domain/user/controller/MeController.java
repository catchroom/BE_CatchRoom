...
public class MeController {
    ...
    public ResponseEntity<ApiResponse<String>> accessToken(HttpServletRequest request) {
        ...
        //redis에 있는 지 확인
        Boolean tokenExists = redisTemplate.hasKey(refreshToken);
        ...
        try {
            ...
            // Token이 만료되었는지 확인
            if (meJWTService.isTokenExpired(jwtPayload)) {
                throw new UserException(ErrorCode.USER_REFRESHTOKEN_EXPIRE);
            }
            ...
            return ResponseEntity.ok(ApiResponse.create(200, newAccessToken));
